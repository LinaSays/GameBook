...
module.exports = {
  ...
  getCategory: (req, res) => {
    const query = 'SELECT S.*, C.name as category, U.name as author FROM story S LEFT JOIN story_has_category H ON S.id = H.story_id LEFT JOIN category C ON H.category_id = C.id LEFT JOIN user U ON S.author_id = U.id WHERE C.id=?';
